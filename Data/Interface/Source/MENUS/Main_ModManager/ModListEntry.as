package
{
	...
	public class ModListEntry extends BSUIComponent
	{
		...
		public static const MAX_TEXT_LEN:uint = 35;
		...
		public function set dataObj(param1:Object) : *
		{
			...
			else if(this._DataObj != null && param1.imageTextureName != this._DataObj.imageTextureName)
			{
				this.UnloadThumbnail();
			}
		...
		public function UnloadThumbnail() : *
		{
			if(this._DataObj != null)
			{
				if(this._DataObj.imageLoadState == ILS_DOWNLOADING || this._DataObj.imageLoadState == ILS_DOWNLOADED)
				{
		...
		override public function redrawUIComponent() : void
		{
			...
			else
			{
				...
				this.RatingHolder_mc.y = this.textField.y + _loc2_.height * Math.min(this.textField.numLines,2) + this.TEXT_STAR_INTERSTIT + (this.textField.numLines > 1?2:0);
				if(this.textField.numLines > 2 && _loc1_.length >= MAX_TEXT_LEN)
				{
					_loc5_ = 0;
					_loc6_ = false;
					_loc7_ = 0;
					while(!_loc6_ && _loc7_ < _loc1_.length)
					{
						_loc8_ = this.textField.getLineIndexOfChar(_loc7_);
						if(_loc8_ == 1)
						{
							_loc5_ = _loc7_;
						}
					...
					_loc1_ = _loc1_.substr(0,_loc5_ - 2) + "...";
					GlobalFunc.SetText(this.textField,_loc1_,false);
				}
				_loc3_ = 0;
				...
				if((this._DataObj.installed == true || this._DataObj.installQueued == true) && this._DataObj.uninstallQueued != true)
				{
					...
					_loc4_.x = _loc3_ - _loc4_.width;
					_loc9_ = _loc4_.transform.colorTransform;
					_loc9_.redOffset = this._DataObj.installQueued == true?Number(-128):Number(0);
					_loc9_.greenOffset = this._DataObj.installQueued == true?Number(-128):Number(0);
					_loc9_.blueOffset = this._DataObj.installQueued == true?Number(-128):Number(0);
					_loc4_.transform.colorTransform = _loc9_;
					_loc3_ = _loc3_ - (_loc4_.width + this.ICON_SPACING);
					this.IconHolder_mc.addChild(_loc4_);
				}
				this.visible = true;
