package
{
	...
	public class Main_LoginHelper extends ISubmenu
	{
		...
		{
			var framesToWait:uint = 0;
			var tempCodeObj:Object = null;
			var executeDelayedOnClose:* = undefined;
			...
			framesToWait = 5;
			tempCodeObj = this.codeObj;
			executeDelayedOnClose = function delayedFunc(param1:Event):void
			{
				if(framesToWait == 0)
				{
					removeEventListener(Event.ENTER_FRAME,executeDelayedOnClose);
					tempCodeObj.onLoginScreenClose();
					tempCodeObj = null;
				}
				else
				{
			...
			addEventListener(Event.ENTER_FRAME,executeDelayedOnClose);
		}
		...
		override public function ProcessUserEvent(param1:String, param2:Boolean) : Boolean
		{
			...
			if(!param2 && !_loc3_)
			{
				...
				{
					case "DeleteSave":
						if(this.Login_CreateNewButton.ButtonVisible && this.iPlatform != PlatformChangeEvent.PLATFORM_PC_KB_MOUSE)
						{
							this.onLogin_CreateNew();
							_loc3_ = true;
						}
		...
		{
			if(this.Login_AcceptButton.ButtonVisible && this.Login_AcceptButton.ButtonEnabled)
			{
				if(this.LoginPage_mc.visible)
				{
