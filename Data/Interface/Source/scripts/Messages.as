   ...
   public dynamic class Messages extends BSUIComponent
   {
      ...
      public function UpdatePositions() : *
      {
         ...
         if(_loc1_ == 1)
         {
            ...
            else if(this.MessageArray.length == 0 && !this.fadingOutMessage && Boolean(_loc2_.CanFadeOut()))
            {
               _loc2_.FadeOut();
               this.bAnimating = false;
         ...
         else if(_loc1_ > 1)
         {
            ...
            this.bAnimating = _loc9_ > 0 || Boolean(_loc4_.bIsDirty);
            if(!_loc4_.bIsDirty)
            {
               _loc10_ = Boolean(_loc4_.CanFadeIn()) && height + _loc4_.height + this.MessageSpacing < this._maxClipHeight;
               if(_loc10_)
               {
                  _loc4_.FadeIn();
               }
               if(Boolean(this.bAnimating) && Boolean(_loc4_.fadeInStarted))
               {
                  _loc11_ = 1;
                  _loc12_ = 0;
               ...
               {
                  _loc13_ = !_loc10_ && Boolean(_loc4_.CanFadeIn());
                  if(!this.bqueuedMessage || _loc1_ == MAX_SHOWN || Boolean(_loc13_) && Boolean(this.ShownMessageArray[0].CanFadeOut()))
                  {
                     this.fadingOutMessage = true;
      ...
      public function Update() : *
      {
         ...
         if(Boolean(this.bqueuedMessage) && Boolean(this.CanAddMessage))
         {
