package Shared.AS3.COMPANIONAPP
{
   ...
   public class SwipeZone extends EventDispatcher
   {
      ...
      protected function mouseMoveHandler(param1:MouseEvent) : void
      {
         ...
         {
            if(Boolean(isNaN(this._mousePressPoint.x)) || Boolean(isNaN(this._mousePressPoint.y)))
            {
               this._mousePressPoint = new Point(param1.stageX,param1.stageY);
            }
      ...
      protected function mouseReleaseHandler(param1:MouseEvent) : void
      {
         ...
         if(Boolean(this._isPressed) && (this._zone == null || this._mousePressPoint.x >= this._zone.x && this._mousePressPoint.x <= this._zone.x + this._zone.width && this._mousePressPoint.y >= this._zone.y && this._mousePressPoint.y <= this._zone.y + this._zone.height))
         {
