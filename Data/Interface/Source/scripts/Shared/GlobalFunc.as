   ...
   public class GlobalFunc
   {
      ...
      public static function MaintainTextFormat() : *
      {
         TextField.prototype.SetText = function(param1:String, param2:Boolean, param3:Boolean = false):*
         {
            ...
            if(Boolean(param3) && param1.charAt(0) != "$")
            {
               param1 = param1.toUpperCase();
            }
            var _loc4_:TextFormat = GlobalFunc.getTextFormat();
            if(param2)
            {
               _loc5_ = Number(_loc4_.letterSpacing);
               _loc6_ = _loc4_.kerning;
               GlobalFunc.htmlText = param1;
               _loc4_ = GlobalFunc.getTextFormat();
               _loc4_.letterSpacing = _loc5_;
               _loc4_.kerning = _loc6_;
               GlobalFunc.setTextFormat(_loc4_);
               GlobalFunc.htmlText = param1;
            }
            else
            {
               GlobalFunc.text = param1;
               GlobalFunc.setTextFormat(_loc4_);
               GlobalFunc.text = param1;
            }
         };
      ...
      public static function SetText(param1:TextField, param2:String, param3:Boolean, param4:Boolean = false, param5:* = false) : *
      {
         ...
         if(Boolean(param4) && param2.charAt(0) != "$")
         {
         ...
         if(Boolean(param5) && param1.text.length > MAX_TRUNCATED_TEXT_LENGTH)
         {
      ...
      public static function AddMovieExploreFunctions() : *
      {
         MovieClip.prototype.getMovieClips = function():Array
         {
            ...
            for(_loc2_ in GlobalFunc)
            {
               if(GlobalFunc[_loc2_] is MovieClip && GlobalFunc[_loc2_] != GlobalFunc)
               {
                  _loc1_.push(GlobalFunc[_loc2_]);
               }
            }
            return _loc1_;
         };
         MovieClip.prototype.showMovieClips = function():*
         {
            ...
            for(_loc1_ in GlobalFunc)
            {
               if(GlobalFunc[_loc1_] is MovieClip && GlobalFunc[_loc1_] != GlobalFunc)
               {
                  trace(GlobalFunc[_loc1_]);
                  GlobalFunc[_loc1_].showMovieClips();
               }
            }
         };
      }
