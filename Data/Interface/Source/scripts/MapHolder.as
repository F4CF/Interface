...
//====================================================================================================
		...
		protected function onMouseMovement(e:MouseEvent) : *
		{
		...
		protected function onMouseUpDetect(e:MouseEvent) : *
		{
...
//====================================================================================================
		...
		override protected function onPipboyChangeEvent(e:PipboyChangeEvent) : void
		{
			var _loc2_:* = this.visible;
			super.onPipboyChangeEvent(e);
			...
			if(!_loc2_)
			{
				this.disableInput = e.DataObj.CurrentTab != this.TabIndex;
			}
			if(_loc3_ === true)
			{
		...
		public function SetTexture(aDisplay:DisplayObject) : *
		{
			this.TextureHolder_mc.addChild(aDisplay);
		...
		protected function onMouseWheel(e:MouseEvent) : *
		{
			if(!this.bDisableInput)
			{
				this.ZoomMap(e.delta * this.MOUSE_WHEEL_ZOOM_SPEED);
				this.ZoomMap(0);
			}
		}


		public function ClearMarkers(e:PipboyChangeEvent) : *
		{
			var _loc2_:Array = null;
			var _loc3_:uint = 0;
			var _loc4_:uint = 0;
			var _loc5_:MovieClip = null;
			if(true || Boolean(e.DataObj.RemoveAllMapMarkers))
			{
			...
			{
				_loc2_ = e.DataObj.RemovedMapMarkerIds;
				_loc3_ = 0;
		...
		{
			if(Boolean(this.bMapLoaded) && Boolean(this.bTexturesRegistered) && !this.bDisableInput && Boolean(this._HasCaravans))
			{
		...
		private function FindClosestMarkerIndex() : uint
		{
			...
			while(_loc6_ < this.MarkerInfoA.length)
			{
				...
				if(Boolean(this.MarkerInfoA[_loc6_].clip.visible) && Boolean(_loc2_.containsPoint(_loc4_)))
				{
...
//====================================================================================================
		...
		{
			if(!this.bTexturesRegistered && Boolean(param1))
			{
		...
		protected function set uiSelectedMarkerIndex(param1:uint) : void
		{
			...
			if(true)
			{
				...
				else if(_loc4_ != this._uiSelectedMarkerId)
				{
					...
					this._bCanFastTravel = !_loc3_.undiscovered && Boolean(_loc5_) && Boolean(this.codeObj.CheckHardcoreModeFastTravel(this._uiSelectedMarkerId));
					this._bCanPlaceMarker = !this._bCanFastTravel && Boolean(_loc5_) || MarkerTypesA[_loc3_.type] == PlayerSetMarker;
				}
				if(Boolean(this.codeObj) && Boolean(_loc2_))
				{
		...
		protected static const MarkerTypesA:Array =
		[
			CaveMarker,
			CityMarker,
			DiamondCityMarker,
			...
			CustomHouseMarker,
			DriveInMarker,
			ElevatedHighwayMarker,
			FaneuilHallMarker,
			FarmMarker,
			FillingStationMarker,
			ForestedMarker,
			GoodneighborMarker,
			GraveyardMarker,
			HospitalMarker,
			IndustrialDomeMarker,
			IndustrialStacksMarker,
			InstituteMarker,
			IrishPrideMarker,
			JunkyardMarker,
			ObservatoryMarker,
			...
			PoliceStationMarker,
			PrydwenMarker,
			RailroadFactionMarker,
			RailroadMarker,
			SatelliteMarker,
			SentinelMarker,
			USSConstitutionMarker,
			MechanistMarker,
			RaiderSettlementMarker,
			VassalSettlementMarker,
			PotentialVassalSettlementMarker,
			BottlingPlantMarker,
			GalacticMarker,
			HubMarker,
			KiddieKingdomMarker,
