package
{
   ...
   public dynamic class HUDObjectiveUpdates extends BSUIComponent
   {
      ...
      {
         if(Boolean(this.CanAddMessage) && this.ObjectiveDataV.length > 0 && this.ObjectiveItemPoolV.length > 0)
         {
         ...
         if(this.ShownObjectivesV.length > 0)
         {
            ...
            if(!this.MostRecentItem.fadeInStarted && Boolean(this.CanFadeInMostRecent))
            {
               this.MostRecentItem.FadeIn();
            }
            if(!this.ObjectivesScrolling)
            {
               if(Boolean(this.BottomItem.CanFastFadeOut()) && this.ObjectiveItemPoolV.length == 0 && this.ObjectiveDataV.length > 0)
               {
