package scaleform.clik.controls
{
	...
	public class ButtonBar extends UIComponent
	{
		...
		public function set itemRendererName(param1:String) : void
		{
			...
			if(Boolean(_inspector) && value == "Button" || value == "")
			{
				return;
			}
			try
		...
		override public function handleInput(e:InputEvent) : void
		{
			...
			if(_loc2_ != null)
			{
				_loc2_.handleInput(e);
				if(e.handled)
				{
					return;
				}
			}
			var _loc3_:InputDetails = e.details;
		...
		{
			if(Boolean(isInvalid(InvalidationType.RENDERERS)) || Boolean(isInvalid(InvalidationType.DATA)) || Boolean(isInvalid(InvalidationType.SETTINGS)) || Boolean(isInvalid(InvalidationType.SIZE)))
			{
		...
		protected function handleButtonGroupChange(e:Event) : void
		{
			if(this._group.selectedIndex != this.selectedIndex)
			{
				this.selectedIndex = this._group.selectedIndex;
				dispatchEvent(new ButtonBarEvent(ButtonBarEvent.BUTTON_SELECT, false, true, this._selectedIndex, e.target as Button));
			}
		}


		protected function handleDataChange(e:Event) : void
		{
