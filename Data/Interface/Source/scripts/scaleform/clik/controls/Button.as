package scaleform.clik.controls
{
	...
	public class Button extends UIComponent
	{
		...
		override public function set enabled(param1:Boolean) : void
		{
			...
			if(super.enabled)
			{
				_loc2_ = this._focusIndicator == null && (Boolean(_displayFocus) || Boolean(_focused))?"over":"up";
			}
			else
			{
				_loc2_ = "disabled";
		...
		public function set selected(param1:Boolean) : void
		{
			...
			if(this.enabled)
			{
				...
				else if(Boolean(this._pressedByKeyboard) && this._focusIndicator != null)
				{
				...
				if(this.owner)
				{
					_loc2_ = Boolean(this._selected) && this.owner != null && Boolean(this.checkOwnerFocused());
					this.setState(Boolean(_loc2_) && this._focusIndicator == null?"selecting":"toggle");
					displayFocus = _loc2_;
				}
			}
			else
		...
		override protected function configUI() : void
		{
			...
			super.configUI();
			tabEnabled = Boolean(_focusable) && Boolean(this.enabled) && Boolean(tabEnabled);
		...
		override protected function changeFocus() : void
		{
			...
			{
				this.setState(Boolean(_focused) || Boolean(_displayFocus)?"over":"out");
				if(Boolean(this._pressedByKeyboard) && !_focused)
				{
			...
			else
			{
				...
				else
				{
					...
					{
						this._newFocusIndicatorFrame = Boolean(_focused) || Boolean(_displayFocus)?"show":"hide";
					}
					invalidateState();
				}
				if(Boolean(this._pressedByKeyboard) && !_focused)
		...
		protected function handleMouseRollOver(param1:MouseEvent) : void
		{
			...
			if(param1.buttonDown)
			{
				...
				if(Boolean(this.lockDragStateChange) && Boolean(Boolean(this._mouseDown << _loc3_ & 1)))
				{
					return;
				}
		...
		protected function handleMouseRollOut(param1:MouseEvent) : void
		{
			...
			if(param1.buttonDown)
			{
				...
				if(Boolean(this.lockDragStateChange) || !this.enabled)
				{
					return;
				}
				if(Boolean(_focused) || Boolean(_displayFocus))
		...
		protected function handleMousePress(param1:MouseEvent) : void
		{
			...
			{
				this.setState("down");
				if(Boolean(this.autoRepeat) && this._repeatTimer == null)
				{
		...
		{
			if(Boolean(this.toggle) && (!this.selected || Boolean(this.allowDeselect)))
			{
