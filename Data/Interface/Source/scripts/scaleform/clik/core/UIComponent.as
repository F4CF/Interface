package scaleform.clik.core
{
	...
	public class UIComponent extends MovieClip
	{
		...
		public static function generateLabelHash(aMovieClip:MovieClip) : Object
		{
			var objHash:Object = {};
			if(!aMovieClip)
			{
			...
			var labels:Array = aMovieClip.currentLabels;
			var labelsLen:uint = labels.length;
			var idx:uint = 0;

			while(idx < labelsLen)
			{
				objHash[labels[idx].name] = true;
				idx++;
			}
			return objHash;
		}
				...
				while(_loc4_ < _loc2_)
				{
					...
					if(_loc5_)
					{
						...
						while(_loc7_ < _loc3_)
						{
							...
							if(Boolean(_loc8_) && FocusManager.getFocus(_loc7_) != this)
							{
		...
		public function handleInput(e:InputEvent) : void
		{
		}


		public function dispatchEventToGame(e:Event) : void
		{
			ExternalInterface.call("__handleEvent", name, e);
		}
		...
		protected function handleStageChange(e:Event) : void
		{
		...
		protected function handleEnterFrameValidation(e:Event) : void
		{
			this.validateNow();
		}
		...
		public function dispatchEventAndSound(e:Event) : Boolean
		{
			var _loc2_:Boolean = super.dispatchEvent(e);
			return _loc2_;
		}
