package
{
   ...
   public class RadioWave extends MovieClip
   {
      ...
      private function GetWavePoint() : Number
      {
         ...
         if(Boolean(this.bshowWinningGroup) && Boolean(this.bwinningGroupFound))
         {
         ...
         var _loc2_:Number = 2 * Math.PI * (Boolean(this.bshowWinningGroup) && Boolean(this.bwinningGroupFound)?_loc1_ + this.randFreqOffset:(!!this.bwrongWord?BAD_FREQ:this.freq) + this.randFreqOffset) * this.WaveIndex / WAVE_WIDTH + this.offset;
         ...
         var _loc4_:Number = MAX_AMP - (!this.bshowWinningGroup && Boolean(this.bwrongWord)?MAX_AMP:_loc3_) * Math.sin(_loc2_);
      ...
      public function Animate() : *
      {
         ...
         this.graphics.clear();
         this.graphics.lineStyle(2,16777215,Boolean(this.bshowWinningGroup) && Boolean(this.bwrongWord)?Number(this.pulsingAlpha):Number(this.darkness / 100));
